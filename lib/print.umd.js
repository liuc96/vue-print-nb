...
var printarea_default = /*#__PURE__*/function () {
  ...
  _createClass(_default, [{
    ...
    value: function init() {
      ...
      var _this = this; // 如果是异步的


      if (this.settings.asyncUrl) {
        _this.settings.asyncUrl(function (url) {
          var PrintAreaWindow = _this.getPrintWindow(url); // 创建iframe
      ...
      var PrintAreaWindow = this.getPrintWindow(url); // 创建iframe
    ...
    value: function beforeHanler(elsdom) {
      var canvasList = elsdom.querySelectorAll('canvas'); // canvas转换png图片
    ...
    value: function getFormData(ele) {
      ...
      for (var _i = 0; _i < copiedInputs.length; _i++) {
        ...
        var copiedInput = copiedInputs[_i]; // 获取select标签
    ...
    value: function previewBox() {
      ...
      previewContent.style.zIndex = this.settings.zIndex; // 打印预览弹窗的header
      ...
      previewContent.appendChild(previewHeader); // close关闭按钮
      ...
      previewHeader.appendChild(close); // 打印预览弹窗的body
      ...
      previewContent.appendChild(previewBody); // 打印预览弹窗的body的工具栏
      ...
      previewBody.appendChild(previewBodyUtil); // 打印的按钮
      ...
      previewBodyUtil.appendChild(previewBodyUtilPrintBtn); // 添加整个预览到body
    ...
    value: function Iframe(url) {
      var frameId = this.settings.id; // 局部打印 用当前的时间做ifrmae的url
      ...
      try {
        ...
        if (!this.settings.preview) {
          ...
          iframe.setAttribute('style', 'border: 0px;flex: 1;'); // 预览打印

          var previewBox = this.previewBox();
          var previewBody = previewBox.previewBody;
          var close = previewBox.close; // 添加ifrmae到预览弹窗
